...
__POSTGRESQL_GERNARATOR__ = {
	...
	ModificationType.CHANGE_LENGTH : lambda t, n, c : f"ALTER TABLE {t} ALTER COLUMN {n} {generateColumn(c)}",
...
class Modification :
	...
	def changeLength(self, name:str, length:int) :
		...
		self.column.append((ModificationType.CHANGE_LENGTH, name, existingColumn))
	
	def addIndex(self, name:str) :
		if name not in self.meta :
			raise ValueError(f'Column name {name} does not exist and cannot be dropped.')
		self.column.append((ModificationType.ADD_INDEX, name))
