from xerial.session.DBSessionBase import DBSessionBase
from xerial.round_robin_connector.RoundRobinConnector import RoundRobinConnector
...
class DBSessionPool :
	def __init__(self, config):
		...
		if self.vendor == Vendor.MARIADB :
			from xerial.session.synchronous.MariaDBSession import MariaDBSession
			self.sessionInstance = MariaDBSession
		elif self.vendor == Vendor.ORACLE :
			from xerial.session.synchronous.OracleDBSession import OracleDBSession
			self.sessionInstance = OracleDBSession
		elif self.vendor == Vendor.POSTGRESQL :
			from xerial.session.synchronous.PostgresDBSession import PostgresDBSession
			...
			from xerial.session.synchronous.MSSQLDBSession import MSSQLDBSession
