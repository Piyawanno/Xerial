...
def run():
	XerialPrimaryCreatorCLI().run(sys.argv[1:])

class XerialPrimaryCreatorCLI:
	def __init__(self):
	...
	def run(self, argv: List[str]):
		...
		self.config = XerialPrimaryCreatorCLI.loadConfig()
	...
	async def createPrimary(self):
		...
		for table in tableList:
			...
			try :
				...
				print(f"TABLE {table}.id cannot add primary.")
